...
class RouterState {
  ...
  currentUserRoute = $derived(this.userRoutes.find(userRoute => {
    if (this.curPathParts.length !== userRoute.pathParts.length) return false;
    for (let i = 0; i < this.curPathParts.length; ++i) {
      if (isPathParam(userRoute.pathParts[i])) continue;
      if (this.curPathParts[i] !== userRoute.pathParts[i]) return false;
    }
  ...
  navigate(path: string, queryParams?: QueryParams): void {
    ...
    history.pushState(null, "", basePath + "/" + newPath + serializeQueryParameters(queryParams));

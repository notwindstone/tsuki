import type { QueryType } from "@/types/anilist/query.type";
import type { VariablesType } from "@/types/anilist/variables.type";
import { QueryParametersType, ParameterType } from "@/constants/anilist";
import { capitalizeWord } from "@/lib/misc/capitalize-word";
import { formatListToGraphqlFields } from "@/lib/graphql/format-list-to-graphql-fields";
...
export function createAnilistQuery({
  ...
  for (const query of queries) {
    ...
    const aliasedCurrentMediaVariables = Object.fromEntries(
      Object.entries(currentMediaVariables).map(([key, value]) => [`${key}${capitalizedAlias}`, value],
      ),
    );
    const aliasedCurrentPageVariables = Object.fromEntries(
      Object.entries(currentPageVariables).map(([key, value]) => [`${key}${capitalizedAlias}`, value],
      ),
    );
    ...
    const templateQueryFields = typeof fields === "string"
      ? fields
      : formatListToGraphqlFields(fields);
